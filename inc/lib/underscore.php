...
function __underscore ($item = null)
{
    $__ = new __;
...
class __
{
    ...
    public function chain ($item = null)
    {
        ...
        $__ = (isset($this) && isset($this->_chained) && $this->_chained) ? $this : __underscore($item);
        $__->_chained = true;
        return $__;
    }
    ...
    public function map ($collection = null, $iterator = null)
    {
        ...
        if (is_null($collection)) return self::_wrap([]);

        $collection = (array)self::_collection($collection);
        if (count($collection) === 0) self::_wrap([]);
    ...
    public function range ($stop = null)
    {
        ...
        switch ($num_args) {
            case 1:
                list($start, $stop, $step) = [0, $args[0], 1];
                break;
            case 2:
                list($start, $stop, $step) = [$args[0], $args[1], 1];
                if ($stop < $start) return self::_wrap([]);
                break;
            default:
                list($start, $stop, $step) = [$args[0], $args[1], $args[2]];
                if ($step > 0 && $step > $stop) return self::_wrap([$start]);
        }
    ...
    public function zip ($array = null)
    {
        ...
        foreach ( $return_arrays as $k => $v ) {
            if (!is_array($return_arrays[ $k ])) $return_arrays[ $k ] = [];
    ...
    public function wrap ($function = null, $wrapper = null)
    {
        ...
        return self::_wrap(function () use ($wrapper, $function) {
            $args = array_merge([$function], func_get_args());
